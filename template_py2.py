...
originally from WebPy project
extracted and modified by dawn110110@gmail.com into single file

python version 2.X needed.
"""
...
if not PY2:
    raise Exception("Python Version not supported" + str(sys.version_info))
...
def to_string(val):
    r"""Converts `val` so that it is safe for use in Unicode HTML.
        ...
        this is to_string() function in web.py
    """
    if val is None:
        return u''
...
def compile_templates(root):
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        out.write('from template import CompiledTemplate, ForLoop, TemplateResult\n\n')
        if dirnames:
